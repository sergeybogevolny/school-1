...
//Ajax file
var id = CLIENT_ID ;
var AJAX = 'documents/mugshot-upload.php?id='+id;
			...
			success: function(response) {
				...
				if (response.error!='' || response.msg=='')
					...
					removeSelection();
					  ...
					  $.get("classes/client_avatar.class.php", { mugshot: 1, client_id: response.id, file:response.file })
 										.done(function(data) {
												//alert.removeClass('alert-error').addClass('alert-success').show().children('span').html('Your image has been saved.');
													window.location = "client.php?id="+response.id ;
...
function webcamSnapshot() {
	removeSelection();
    $('#webcam_snapshot').hide();
	$(uploadC).hide();
	$(webcamC).find('.crop').html('');
	webcam.set_api_url(AJAX);
	webcam.set_swf_url('documents/mugshot/assets/webcam/webcam.swf');
	webcam.set_shutter_sound(true, 'documents/mugshot/assets/webcam/shutter.mp3');  // play shutter click sound
		...
		onComplete: function(data, response){
			if(response.error!='' || response.msg=='') {
				if( response.error.ext )
					alert.addClass('alert-error').show().children('span').text('File extension not allowed.' + response.error.ext );
