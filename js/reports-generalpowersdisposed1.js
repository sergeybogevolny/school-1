...
function loadResult(){
    ...
    oTable.fnClearTable();
			...
			$.get("classes/reports_generalpowersdisposed1.class.php",{ value:'sql'}, function(data) {
					...
					if (data.match('id="error"') !== null) {
					 ...
					 for (var i in jsonData) {
						    ...
						    $('#campaign-table').show().dataTable().fnAddData([
								  ...
								  rec.executed,
								  rec.prefix +'-'+ rec.serial,
								  rec.defendant,
								  rec.disposedate,
								  rec.amount
								  
								]);
						 $('#reportbutton').html('<a href="forms/generalpowerdispose.php" target="_blank"><button class="btn btn-small btn-primary"><i class=" icon-print"></i> Report</button></a></center>')
