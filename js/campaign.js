...
function loadResult(){
    ...
    oTable.fnClearTable();
			...
			$.get("classes/campaign.class.php",{ value:'sql'}, function(data) {
					...
					if (data.match('id="error"') !== null) {
					 ...
					 for (var i in jsonData) {
							var rec = jsonData[i];
							var isemail = (rec.is_email == 1 ) ? '' : 'disabled="disabled"' ;
							var ismessage = (rec.is_message == 1 ) ? '' : 'disabled="disabled"' ;
							var isautomatedcall = (rec.is_automatedcall == 1 ) ? '' : 'disabled="disabled"' ;
							var isletter = (rec.is_letter == 1 ) ? '' : 'disabled="disabled"' ;


							console.log(rec);
						    $('#campaign-table').show().dataTable().fnAddData([
    		                  '<input type="checkbox" name="isletter" value="0"  onchange=" campaignCheck($(this))"  class="isletter"  '+isletter+'>',
							  '<input type="checkbox"  name="isemail" value="0"  onchange=" campaignCheck($(this))"  class="isemail" '+isemail+' >',
    		                  '<input type="checkbox" name="ismessage" value="0"  onchange=" campaignCheck($(this))" class="ismessage"  '+ismessage+'>',
    		                  '<input type="checkbox" name="isautomatedcall" value="0"  onchange=" campaignCheck($(this))" class="isautomatedcall"  '+isautomatedcall+'>',
								  rec.first,
								  rec.last,
								  rec.balance,
								]);
