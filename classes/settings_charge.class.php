...
class Settings_charge extends Generic {
	...
	private function validate() {
	    if(empty($this->options['charge-name'])) {
    ...
    private function unique() {
        ...
        global $generic;
        $sql = $sql = "SELECT * FROM agency_settings_lists_charges WHERE name = '" . $this->options['charge-name'] . "' AND flag = 0";
    ...
    private function flag() {
		...
		$params = array(
            ':flag' => 1,
            ':id' => $this->options['charge-id']
		);

		$sql = "UPDATE `agency_settings_lists_charges` SET `flag` = :flag WHERE `id` = :id;";
    ...
    private function add() {
		...
		$params = array(
            ':name'    => $this->options['charge-name']
		);

        $sql = "INSERT INTO `agency_settings_lists_charges` (`name`) VALUES (:name);";
    ...
    private function edit() {
		...
		$sql = "UPDATE `agency_settings_lists_charges` SET `name` = :name WHERE `id` = :id;";
    ...
    private function grab() {
		...
		$stmt   = parent::query("SELECT * FROM agency_settings_lists_charges WHERE id = :id;", $params);
...
$settingscharge = new Settings_charge();
