...
class Settings_setfor extends Generic {
	...
	private function validate() {
	    if(empty($this->options['setfor-name'])) {
    ...
    private function unique() {
        ...
        global $generic;
        $sql = $sql = "SELECT * FROM agency_settings_lists_setfors WHERE name = '" . $this->options['setfor-name'] . "' AND flag = 0";
    ...
    private function flag() {
		...
		$params = array(
            ':flag' => 1,
            ':id' => $this->options['setfor-id']
		);

		$sql = "UPDATE `agency_settings_lists_setfors` SET `flag` = :flag WHERE `id` = :id;";
    ...
    private function add() {
		...
		$params = array(
            ':name'    => $this->options['setfor-name']
		);

        $sql = "INSERT INTO `agency_settings_lists_setfors` (`name`) VALUES (:name);";
    ...
    private function edit() {
		...
		$sql = "UPDATE `agency_settings_lists_setfors` SET `name` = :name WHERE `id` = :id;";
    ...
    private function grab() {
		...
		$stmt   = parent::query("SELECT * FROM agency_settings_lists_setfors WHERE id = :id;", $params);
...
$settingssetfor = new Settings_setfor();
