...
class Settings_office extends Generic {
	...
	private function validate() {
	    if(empty($this->options['office-name'])) {
    ...
    private function unique() {
        ...
        global $generic;
        $sql = $sql = "SELECT * FROM agency_settings_lists_offices WHERE name = '" . $this->options['office-name'] . "' AND flag = 0";
    ...
    private function flag() {
		...
		$params = array(
            ':flag' => 1,
            ':id' => $this->options['office-id']
		);

		$sql = "UPDATE `agency_settings_lists_offices` SET `flag` = :flag WHERE `id` = :id;";
    ...
    private function add() {
		...
		$params = array(
            ':name'    => $this->options['office-name']
		);

        $sql = "INSERT INTO `agency_settings_lists_offices` (`name`) VALUES (:name);";
    ...
    private function edit() {
		...
		$sql = "UPDATE `agency_settings_lists_offices` SET `name` = :name WHERE `id` = :id;";
    ...
    private function grab() {
		...
		$stmt   = parent::query("SELECT * FROM agency_settings_lists_offices WHERE id = :id;", $params);
...
$settingsoffice = new Settings_office();
