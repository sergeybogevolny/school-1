...
class Settings_source extends Generic {
	...
	private function validate() {
	    if(empty($this->options['source-name'])) {
    ...
    private function unique() {
        ...
        global $generic;
        $sql = $sql = "SELECT * FROM agency_settings_lists_sources WHERE name = '" . $this->options['source-name'] . "' AND flag = 0";
    ...
    private function flag() {
		...
		$sql = "UPDATE `agency_settings_lists_sources` SET `flag` = :flag WHERE `id` = :id;";
    ...
    private function add() {
        ...
        $sql = "INSERT INTO `agency_settings_lists_sources` (`name`) VALUES (:name);";
    ...
    private function edit() {
		...
		$sql = "UPDATE `agency_settings_lists_sources` SET `name` = :name WHERE `id` = :id;";
    ...
    private function grab() {
		...
		$stmt   = parent::query("SELECT * FROM agency_settings_lists_sources WHERE id = :id;", $params);
...
$settingssource = new Settings_source();
