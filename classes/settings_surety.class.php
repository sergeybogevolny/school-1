...
class Settings_surety extends Generic {
	...
	private function validate() {
	    if(empty($this->options['surety-name'])) {
    ...
    private function unique() {
        ...
        global $generic;
        $sql = $sql = "SELECT * FROM agency_settings_lists_sureties WHERE name = '" . $this->options['surety-name'] . "' AND flag = 0";
    ...
    private function flag() {
		...
		$params = array(
            ':flag' => 1,
            ':id' => $this->options['surety-id']
		);

		$sql = "UPDATE `agency_settings_lists_sureties` SET `flag` = :flag WHERE `id` = :id;";
    ...
    private function add() {
		...
		$params = array(
            ':name'    => $this->options['surety-name']
		);

        $sql = "INSERT INTO `agency_settings_lists_sureties` (`name`) VALUES (:name);";
    ...
    private function edit() {
		...
		$sql = "UPDATE `agency_settings_lists_sureties` SET `name` = :name WHERE `id` = :id;";
    ...
    private function grab() {
		...
		$stmt   = parent::query("SELECT * FROM agency_settings_lists_sureties WHERE id = :id;", $params);
...
$settingssurety = new Settings_surety();
