...
class Settings_courier extends Generic {
	...
	private function validate() {
	    if(empty($this->options['courier-name'])) {
    ...
    private function unique() {
        ...
        global $generic;
        $sql = $sql = "SELECT * FROM agency_settings_lists_couriers WHERE name = '" . $this->options['courier-name'] . "' AND flag = 0";
    ...
    private function flag() {
		...
		$params = array(
            ':flag' => 1,
            ':id' => $this->options['courier-id']
		);

		$sql = "UPDATE `agency_settings_lists_couriers` SET `flag` = :flag WHERE `id` = :id;";
    ...
    private function add() {
		...
		$params = array(
            ':name'    => $this->options['courier-name']
		);

        $sql = "INSERT INTO `agency_settings_lists_couriers` (`name`) VALUES (:name);";
    ...
    private function edit() {
		...
		$sql = "UPDATE `agency_settings_lists_couriers` SET `name` = :name WHERE `id` = :id;";
    ...
    private function grab() {
		...
		$stmt   = parent::query("SELECT * FROM agency_settings_lists_couriers WHERE id = :id;", $params);
...
$settingscourier = new Settings_courier();
