...
class MYPDF extends TCPDF {
    ...
    public function Header() {
		...
		$this->Cell(31, 0, 'Date', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Cell(30, 0, 'Time', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Cell(45, 0, 'Name', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Cell(16, 0, 'Class', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Cell(32, 0, 'Charge', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Cell(31, 0, 'Case', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Cell(31, 0, 'County', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Cell(31, 0, 'Court', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Cell(41, 0, 'Set For', 'B', $ln=0, 'L', 0, '', 0, false, 'T', 'C');
		$this->Ln();
        $this->Ln();
...
while( $row = $query->fetch(PDO::FETCH_ASSOC) ) {
    ...
    $txt_time     	= date('h:i A',strtotime($row['time']));
    $txt_name     	= $row['name'];
    $txt_class      = $row['class'];
    ...
    if ($group==1){
        ...
        if ($grouplastvalue!=$by){
            ...
            $pdf->SetX(250);
           // $pdf->Cell(41, 0, $txt_groupsub1_us, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
            $pdf->Ln();
            //$pdf->Rect(250, $gy, 41, 0.75, 'DF', $stylesingle, array(420, 420, 400));
            $pdf->Ln();
	...
	$pdf->Cell(31, 0,$txt_date_mdY, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
    $pdf->Cell(30, 0, $txt_time, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
    $pdf->Cell(45, 0, $txt_name, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
    $pdf->Cell(16, 0, $txt_class, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
    $pdf->Cell(32, 0, $txt_charge, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
    $pdf->Cell(31, 0, $txt_casenumber, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
	$pdf->Cell(31, 0, $txt_county, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
    $pdf->Cell(31, 0, $txt_court, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
	$pdf->Cell(41, 0, $txt_setfor, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');

    $pdf->Ln();

    if ($group==1){
        $grouplastvalue = $txt_date;
        $groupsubcount  = $groupsubcount + 1;
        $groupsub1      = $groupsub1 + $txt_setfor;
    }

    if ($total==1){
        $grandtotalcount    = $grandtotalcount + 1;
        $grandtotal1        = $grandtotal1 + $txt_setfor;
    }
...
if ($total==1){
    ...
    $pdf->SetX(250);
    //$pdf->Cell(41, 0, $txt_grandtotal1_us, 0, $ln=0, 'L', 0, '', 0, false, 'T', 'C');
    $pdf->Ln();
    //$pdf->Rect(250, $gy, 41, 0.75, 'DF', $styledouble, array(420, 420, 400));
}
