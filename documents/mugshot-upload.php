...
define('UPLOAD_PATH', dirname(dirname(__FILE__)) ."/documents/mugshot/uploads/");
...
require_once("mugshot/includes/functions.php");

require_once("mugshot/includes/SimpleImage.php");
...
switch ($_POST['action'])
{
	...
	case 'save_image':
		...
		if(isset($_SESSION['_tmp_image'], $_POST['x1'], $_POST['y1'], $_POST['w'], $_POST['h']))
		{
			...
			$path_dir = dirname(dirname(__FILE__)) .'/documents/'. $id ;

			if (!is_dir($path_dir)){

				mkdir($path_dir);
				mkdir( dirname(dirname(__FILE__)) .'/documents/'. $id .'/root/' );

				}

			$path = dirname(dirname(__FILE__)) .'/documents/'. $id .'/root/' ;

			$tmp_path .= $_SESSION['_tmp_image'];

			//Set new path
			$path .= $_SESSION['_tmp_image'];
			...
			@unlink($tmp_path);
			$path1 = $id .'/root/'.$_SESSION['_tmp_image'];
			$img_name = $_SESSION['_tmp_image'];
			//return image url
			$msg = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['REQUEST_URI'])
				. '/' . $path1 . '?'. time(); //add the current timestamp at the end so the image won't be cached;
	...
	case 'webcam':
		...
		if (!empty($jpeg_data))
		{
			...
			$filename = 'mugshot/uploads/tmp/' . $filename;
			//save image
...
//json response
echo  json_encode(array('error' => $error, 'msg' => $msg ,'id' => $id,'file' => $img_name));
?>